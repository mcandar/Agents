# US Bubble Map for Receipent Postal Codes, Month10.txt
...
temp <- strsplit(as.character(Raw[,4]),"-") # separate zip info of the form XXXXX-XXXX
ReceipentZips <- as.data.frame(matrix(lapply(temp, "[",1))) # save first five digits as one column matrix
ReceipentZips <- trimws(ReceipentZips[[1]]) # trim white spaces for searching
...
Arranged$Region <- rbind(matrix(3,nrow(Arranged)/3,1),       # Divide into regions
                         matrix(2,(nrow(Arranged)/3)+1,1),   # first region has the highest amount of orders
                         matrix(1,(nrow(Arranged)/3)+1,1))   # third has the lowest
...
p <- plot_ly(Arranged, lon = Lon, lat = Lat, text = notes,
             marker = list(size = sqrt(sqrt(as.numeric(Arranged[,5])/10)) + 1, line = list(width = 0)),
             type = 'scattergeo', locationmode = 'USA-states',color = Region) %>%
  layout(title = 'Oct 2012 US Receipent Distribution<br>(Click legend to toggle)', geo = g)

p # show it

# save as an html page to keep interactive tools
htmlwidgets::saveWidget(as.widget(p), "USA_RECEIPENT_DIST_M10_.html")
