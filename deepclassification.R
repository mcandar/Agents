...
#
### Introduction
#This tutorial shows how a H2O [Deep Learning](http://en.wikipedia.org/wiki/Deep_learning) model can be used to do
# supervised classification and regression. A great tutorial about Deep Learning is given by Quoc Le 
# [here](http://cs.stanford.edu/~quocle/tutorial1.pdf) and [here](http://cs.stanford.edu/~quocle/tutorial2.pdf). 
# This tutorial covers usage of H2O from R. A python version of this tutorial will be available as well in a separate 
# document. This file is available in plain R, R markdown and regular markdown formats, and the plots are available as 
# PDF files. All documents are available [on Github](https://github.com/h2oai/h2o-tutorials/tree/master/tutorials/deeplearning/).
#
...
# dev.off()
#
#Let's investigate some more Deep Learning models. First, we explore the evolution over training
# time (number of passes over the data), and we use checkpointing to continue training the same model:
#
#dev.new(noRStudioGD=FALSE) #direct plotting output to a new window
pdf("test_classf_epoch.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
ep <- c(1,250,500,750)
...
# be required for optimal performance. Next, we compare between different activation functions, 
# including one with 50% dropout regularization in the hidden layers:
#
#dev.new(noRStudioGD=FALSE) #direct plotting output to a new window
pdf("test_classf_params.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
...
dev.off()
#
# Clearly, the dropout rate was too high or the number of epochs was too low 
# for the last configuration, which often ends up performing the best on 
# larger datasets where generalization is important.
#
...
#
df <- h2o.importFile("https://raw.githubusercontent.com/h2oai/h2o-tutorials/master/tutorials/data/covtype.full.csv")
dim(df)
df
...
#
#Here's a scalable way to do scatter plots via binning (works for categorical and numeric columns) 
# to get more familiar with the dataset.
#
#dev.new(noRStudioGD=FALSE) #direct plotting output to a new window
pdf("test_railw_canv.pdf")
...
#We want to predict the `Cover_Type` column, a categorical feature with 7 levels, and the Deep 
# Learning model will be tasked to perform (multi-class) classification. It uses the other 12 
# predictors of the dataset, of which 10 are numerical, and 2 are categorical with a total of 
# 44 levels. We can expect the Deep Learning model to have 56 input neurons (after automatic one-hot encoding).
#
response <- "Cover_Type"
...
summary(m1)
#
#Inspect the model in [Flow](http://localhost:54321/) for more information about model 
# building etc. by issuing a cell with the content `getModel "dl_model_first"`, and pressing Ctrl-Enter.
#
#### Variable Importances
#Variable importances for Neural Network models are notoriously difficult to compute,
# and there are many [pitfalls](ftp://ftp.sas.com/pub/neural/importance.html). H2O 
# Deep Learning has implemented the method of [Gedeon](http://cs.anu.edu.au/~./Tom.
# Gedeon/pdfs/ContribDataMinv2.pdf), and returns relative variable importances in 
# descending order of importance.
#
head(as.data.frame(h2o.varimp(m1)))
#
#### Early Stopping
#Now we run another, smaller network, and we let it stop automatically once the 
# misclassification rate converges (specifically, if the moving average of length
# 2 does not improve by at least 1% for 2 consecutive scoring events). We also 
# sample the validation set to 10,000 rows for faster scoring.
#
m2 <- h2o.deeplearning(
...
#By default, H2O Deep Learning uses an adaptive learning rate 
# ([ADADELTA](http://arxiv.org/pdf/1212.5701v1.pdf)) for its stochastic 
# gradient descent optimization. There are only two tuning parameters 
# for this method: `rho` and `epsilon`, which balance the global and 
# local search efficiencies. `rho` is the similarity to prior weight 
# updates (similar to momentum), and `epsilon` is a parameter that 
# prevents the optimization to get stuck in local optima. Defaults 
# are `rho=0.99` and `epsilon=1e-8`. For cases where convergence speed
# is very important, it might make sense to perform a few runs to 
# optimize these two parameters (e.g., with `rho in c(0.9,0.95,0.99,0.999)`
# and `epsilon in c(1e-10,1e-8,1e-6,1e-4)`). Of course, as always with
# grid searches, caution has to be applied when extrapolating grid 
# search results to a different parameter regime (e.g., for more 
# epochs or different layer topologies or activation functions, etc.).
#
...
#
#### Tuning
#With some tuning, it is possible to obtain less than 10% test set error 
# rate in about one minute. Error rates of below 5% are possible with larger 
# models. Note that deep tree methods can be more effective for this dataset 
# than Deep Learning, as they directly partition the space into sectors, which seems to be needed here.
#
m3 <- h2o.deeplearning(
...
#
#To confirm that the reported confusion matrix on the validation set 
# (here, the test set) was correct, we make a prediction on the test 
# set and compare the confusion matrices explicitly:
#
...
summary(m_loaded)
#
#This model is fully functional and can be inspected, restarted, or used to score a 
# dataset, etc. Note that binary compatibility between H2O versions is currently not guaranteed.
#
####Cross-Validation
#For N-fold cross-validation, specify `nfolds>1` instead of (or in addition to) a validation frame,
# and `N+1` models will be built: 1 model on the full training data, and N models with each 1/N-th 
# of the data held out (there are different holdout strategies). Those N models then score on the 
# held out data, and their combined predictions on the full training data are scored to get the 
...
dlmodel
#
#N-fold cross-validation is especially useful with early stopping, as the main model will pick 
# the ideal number of epochs from the convergence behavior of the cross-validation models.
#
###Regression and Binary Classification
...
# that contains only 1 output neuron (instead of 2). The reason is that the response was a 
# numerical feature (ordinal numbers 0 and 1), and H2O Deep Learning was run with 
# `distribution=AUTO`, which defaulted to a Gaussian regression problem for a real-valued response.
# H2O Deep Learning supports regression for distributions other than 
# `Gaussian` such as `Poisson`, `Gamma`, `Tweedie`, `Laplace`. It also supports
# `Huber` loss and per-row offsets specified via an `offset_column`. We refer to 
# our [H2O Deep Learning regression code examples]
# (https://github.com/h2oai/h2o-3/tree/master/h2o-r/tests/testdir_algos/deeplearning) for more information.
#
...
pdf("test_dl_binary_classf_ROC.pdf")
...
# demo(h2o.deeplearning)  #requires user interaction
#
# While H2O Deep Learning has many parameters, it was designed to be just as easy to use as the other
# supervised training methods in H2O. Early stopping, automatic data standardization and handling of 
# categorical variables and missing values and adaptive learning rates (per weight) reduce the amount 
# of parameters the user has to specify. Often, it's just the number and sizes of hidden layers, the 
# number of epochs and the activation function and maybe some regularization techniques.
#
#### Let's have some fun first: Decision Boundaries
#We start with a small dataset representing red and black dots on a plane, arranged in the shape of two nested spirals. Then we task H2O's machine learning methods to separate the red and black dots, i.e., recognize each spiral as such by assigning each point in the plane to one of the two spirals.
#
#We visualize the nature of H2O Deep Learning (DL), H2O's tree methods (GBM/DRF) and H2O's generalized
# linear modeling (GLM) by plotting the decision boundary between the red and black spirals:
...
# real data
# final_matched <- read.csv("final_matched.csv",row.names = NULL)
# final_summary <- read.csv("final_summary.csv",row.names = NULL)

temp <- Sort(subset(final_summary,NumberofUses > 100)[,c(1,3)],2,decreasing = TRUE)
temp$Product <- as.character(temp$Product)

first <- 1     # first item
second <- 150  # 150th item
size <- 100
ind_1 <- sample(nrow(subset(final_matched,ItemDescription == temp$Product[first])),size)
ind_2 <- sample(nrow(subset(final_matched,ItemDescription == temp$Product[second])),size) # use this only

test_set <- rbind(Sort(subset(final_matched,ItemDescription == temp$Product[first]),7,decreasing = TRUE)[1:size,c(7,20,25)],
                  Sort(subset(final_matched,ItemDescription == temp$Product[second]),7)[ind_2,c(7,20,25)])

test_set <- Sort(test_set,1,decreasing = TRUE)

x <- test_set[,2:1]
x$color <- ifelse(test_set$ItemDescription==test_set$ItemDescription[1],"Black","Red")
...
# form a grid
lims <- data.frame(xlim=c(max(x$x),min(x$x)),ylim=c(max(x$y),min(x$y)))
grid_testx <- seq(lims$xlim[1],lims$xlim[2],length.out = 1000)
grid_testy <- seq(lims$ylim[1],lims$ylim[2],length.out = 1000)
...
x <- as.h2o(x) # colnames(x) <- c("Distance","ShippingCost","color") originally
grid_test <- as.h2o(grid_test[1e6:1,]) # reverse the order
# plotC("DL", h2o.deeplearning(1:2,3,x,epochs=1),x,g=grid_test)
x
grid_test
# ad-hoc inputs
name <- "Deep Learning"
model <- h2o.deeplearning(1:2,3,x,epochs=5000)
data <- x
g <- grid_test
...
# d <- 1.5 # axis limits
# h <- d*(-n:n)/n
pdf("TEST_DL_CLASSIFIER_REALDATA_ep_5000.pdf")
plot(data[,-3],pch=19,col=data[,3],cex=0.5,xlim=c(lims$xlim[1],lims$xlim[2]),ylim=c(lims$ylim[1],lims$ylim[2]),main=name)
contour(grid_testx[1000:1],grid_testy[1000:1],z=array(ifelse(pred[,1]=="Red",0,1),dim=c(2*n+1,2*n+1)),col="blue",lwd=2,add=T)
dev.off()
...
pdf("DEEP_CLASSF_METHODS.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
classify.dl("Neural Network", h2o.deeplearning(1:2,3,x,epochs=1e3),x)
classify.dl("Gradient Boost Machine", h2o.gbm(1:2,3,x),x)
classify.dl("Random Forest", h2o.randomForest(1:2,3,x),x)
classify.dl("Generalized Linear Model", h2o.glm(1:2,3,x,family="binomial"),x)
dev.off()

# x_new <- as.h2o(as.data.frame(x)[200:1,])
classify.dl("Neural Network", h2o.deeplearning(1:2,3,x,epochs=1e3),x,"test_to_delete.pdf")

#Let's investigate some more Deep Learning models. First, we explore the evolution over training
# time (number of passes over the data), and we use checkpointing to continue training the same model:
#
#dev.new(noRStudioGD=FALSE) #direct plotting output to a new window
pdf("DEEP_CLASSF_EPOCHS.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
ep <- c(10,100,1000,5000)
classify.dl(paste0("Neural Net ",ep[1]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[1],model_id="dl_1"),x)
classify.dl(paste0("Neural Net ",ep[2]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[2],checkpoint="dl_1",model_id="dl_2"),x)
classify.dl(paste0("Neural Net ",ep[3]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[3],checkpoint="dl_2",model_id="dl_3"),x)
classify.dl(paste0("Neural Net ",ep[4]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[4],checkpoint="dl_3",model_id="dl_4"),x)
dev.off()

#You can see how the network learns the structure of the spirals with enough training time. 
# We explore different network architectures next:

pdf("DEEP_CLASSF_HIDDENS.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
for (hidden in list(c(11,13,17,19),c(42,42,42),c(200,200),c(1000))) {
  classify.dl(paste0("Neural Net hidden=",paste0(hidden, collapse="x")),
        h2o.deeplearning(1:2,3,x,hidden=hidden,epochs=500),x)
}
dev.off()
...
pdf("DEEP_CLASSF_PARAMS.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
for (act in c("Tanh","Maxout","Rectifier","RectifierWithDropout")) {
  classify.dl(paste0("DL ",act," activation"),h2o.deeplearning(1:2,3,x,activation=act,hidden=c(100,100),epochs=1000),x)
}
dev.off()

######

# two of the most ordered items
items <- as.data.frame(Sort(final_summary,5,decreasing = TRUE)[1:5,1])
items <- as.character(items[c(1,4),])
items # "DVD BRN ASUS | DRW-24B1ST/BLK/B/AS%" "CPU INTEL|CORE I5 3570K 3.4G 6M R"  

test_set <- rbind(Sort(subset(final_matched,ItemDescription == items[1]),7,decreasing = TRUE)[,c(7,20,25)],
                  Sort(subset(final_matched,ItemDescription == items[2]),7,decreasing = TRUE)[,c(7,20,25)])
...
classify.dl("Deep Learning epoch = 10",h2o.deeplearning(1:2,3,x,epochs=10)
            ,x,"Deep_Classification_ep10_BIG.pdf")

pdf("DEEP_CLASSF_EPOCHS_BIG.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
ep <- c(10,100,1000,5000)
classify.dl(paste0("Neural Net ",ep[1]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[1],model_id="dl_1"),x)
classify.dl(paste0("Neural Net ",ep[2]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[2],checkpoint="dl_1",model_id="dl_2"),x)
...
pdf("DEEP_CLASSF_METHODS_BIG.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
classify.dl("Neural Network", h2o.deeplearning(1:2,3,x,epochs=1e3),x)
classify.dl("Gradient Boost Machine", h2o.gbm(1:2,3,x),x)
classify.dl("Random Forest", h2o.randomForest(1:2,3,x),x)
...
# two of the bestseller items
head(final_summary)
items <- as.data.frame(Sort(final_summary,6,decreasing = TRUE)[1:5,1])
items
items <- as.character(items[1:2,])
items # "HP UNIVERSAL INKJET BOND PAPER R"    "DVD BRN ASUS | DRW-24B1ST/BLK/B/AS%"
...
classify.dl("Deep Learning epoch = 10",h2o.deeplearning(1:2,3,x,epochs=10)
            ,x,"Deep_Classification_ep10_BIG_BS2.pdf")

pdf("DEEP_CLASSF_EPOCHS_BIG_BS2.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
ep <- c(10,100,1000,5000)
classify.dl(paste0("Neural Net ",ep[1]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[1],model_id="dl_1"),x)
classify.dl(paste0("Neural Net ",ep[2]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[2],checkpoint="dl_1",model_id="dl_2"),x)
...
pdf("DEEP_CLASSF_METHODS_BIG_BS2.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
classify.dl("Neural Network", h2o.deeplearning(1:2,3,x,epochs=1e3),x)
classify.dl("Gradient Boost Machine", h2o.gbm(1:2,3,x),x)
classify.dl("Random Forest", h2o.randomForest(1:2,3,x),x)
...
# two items, one is most profitable other one is worst
head(final_summary)
fs <- final_summary
fs$GrossProfit <- fs$Price*fs$TotalUnitsShipped
fs$Balance <- fs$GrossProfit-fs$TotalShippingCost
temp <- subset(fs,NumberofUses > 500)
items <- as.data.frame(Sort(temp,15,decreasing = TRUE)[c(1,2,3,nrow(temp)-1,nrow(temp)),1])
items
sapply(items[,1], function(x) length(which(as.character(final_matched$ItemDescription)==x)))

items <- as.character(items[c(1,5),])
items # "CPU INTEL|CORE I7 3770K 3.5G 8M R" "HDMI CBL COBOC| HS-6 %"
...
classify.dl("Deep Learning epoch = 10",h2o.deeplearning(1:2,3,x,epochs=10)
            ,x,"Deep_Classification_ep10_BIG_MPWP.pdf")

pdf("DEEP_CLASSF_EPOCHS_BIG_MPWP.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
ep <- c(10,100,1000,5000)
classify.dl(paste0("Neural Net ",ep[1]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[1],model_id="dl_1"),x)
classify.dl(paste0("Neural Net ",ep[2]," epochs"),h2o.deeplearning(1:2,3,x,epochs=ep[2],checkpoint="dl_1",model_id="dl_2"),x)
...
pdf("DEEP_CLASSF_METHODS_BIG_MPWP.pdf")
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
classify.dl("Neural Network", h2o.deeplearning(1:2,3,x,epochs=1e3),x)
classify.dl("Gradient Boost Machine", h2o.gbm(1:2,3,x),x)
classify.dl("Random Forest", h2o.randomForest(1:2,3,x),x)
