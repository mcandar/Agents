...
Shipping_Filtered <- read.csv("Shipping_Filtered_Q3.csv",row.names=NULL) # for last quarter of 2012
Raw_SaleData <- Format.SaleData(read.table("Month8.txt",sep=",",colClasses = "character")) # for month11, as seen.
print("Sale data is imported")

print("I start Match.ShipData() function.\n")
matched_m8 <- Match.ShipData(Shipping_Filtered,month = 8,Raw_SaleData,filename = "Most_Expensive_Matched_M8_test_",begin = 9)